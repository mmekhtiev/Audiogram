...
public class AudioProcessor {
    ...
    public static boolean transcodeTracks(List<Track> tracks) {
        ...
        for (int i = 1; i < tracks.size(); i++) {
            byte[] resData = overlayAudio(result, tracks.get(i), 48000);
            result.setTrackDescription(new TrackDescription());
    ...
    private static byte[] overlayAudio(Track track1, Track track2, int sampleRate) {
        ...
        int startIndex1 = (int) (track1.getTrackDescription().getStartTime() * byteRate);
        int startIndex2 = (int) (track2.getTrackDescription().getStartTime() * byteRate);

        int length1 = startIndex1 + data1.length;
        int length2 = startIndex2 + data2.length;
        int resultLength = length1 > length2 ? length1 : length2;
        ...
        System.arraycopy(data1, 0, resData, startIndex1, data1.length);

        for (int i = startIndex2; i < data2.length; i++) {
            resData[i] = (byte) ((resData[i] + data2[i - startIndex2]));
        }
